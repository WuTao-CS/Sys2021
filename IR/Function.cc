...
void NameBaseBlock(BaseBlock *bb, std::map<Value *, int> &seq, int &print_cnt_)
{
    ...
    if (bb->isBasicBlock())
    {
        ...
        for (auto instr : basicblock->getInstructions())
        {
            ...
            case Instruction::Call:
                std::cout << "Call ";
                std::cout
                    << static_cast<Function *>(instr->getOperand(0))->getName()
                    << " ";
                break;
...
std::string Function::print()
{
    ...
    if ( this->is_declaration() )
    {
        for ( int i = 0 ; i < this->getNumArgs() ; i++)
        {
            if(i)
                func_ir += ", ";
            func_ir += static_cast<FunctionType *>(this->getType())->getArgType(i)->print();
        }
    }
    else
    {
