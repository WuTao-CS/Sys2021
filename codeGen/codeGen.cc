...
std::string
codeGen::generateModuleCode(optional<std::map<Value *, int>> register_mapping)
{
    std::string asm_code;
    if (register_mapping)
    {
    ...
    { // set all virtual register mapped to memory
...
std::string codeGen::generateInstructionCode(Instruction *inst)
{
    ...
    else if (inst->getInstrType() == Instruction::Call)
    {
        ...
        for (int i = 0; i < std::min(ops.size(), (size_t)5); i++)
        {
            ...
            {
                target_func_name =
                    static_cast<Function *>(ops.at(i))->getName();
            }
            else
            {
        ...
        for (int i = ops.size() - 1; i >= 5; i--)
        {
            ...
            asm_code +=
                InstGen::store(InstGen::Reg(op_reg_0),
                               InstGen::Addr(InstGen::sp, -total_args_size));
        }
        asm_code += InstGen::mov(InstGen::Reg(op_reg_0),
                                 InstGen::Constant(total_args_size));
        asm_code +=
            InstGen::sub(InstGen::sp, InstGen::sp, InstGen::Reg(op_reg_0));
        asm_code += InstGen::call(target_func_name);
        asm_code += InstGen::mov(InstGen::Reg(op_reg_0),
                                 InstGen::Constant(total_args_size));
        asm_code +=
            InstGen::add(InstGen::sp, InstGen::sp, InstGen::Reg(op_reg_0));
    ...
    else if (inst->getInstrType() == Instruction::And)
    {
        ...
        asm_code += codeGen::assignToSpecificReg(ops.at(1), op_reg_1);
        asm_code +=
            InstGen::and_(InstGen::Reg(op_reg_0), InstGen::Reg(op_reg_0),
                          InstGen::Reg(op_reg_1));
        asm_code += codeGen::getFromSpecificReg(inst, op_reg_0);
    }
    ...
    else if (inst->getInstrType() == Instruction::Cmp)
    {
        ...
        asm_code += codeGen::assignToSpecificReg(ops.at(1), op_reg_1);
        asm_code +=
            InstGen::cmp(InstGen::Reg(op_reg_0), InstGen::Reg(op_reg_1));
        asm_code += InstGen::mov(InstGen::Reg(op_reg_0), InstGen::Constant(0));
        asm_code += InstGen::mov(InstGen::Reg(op_reg_0), InstGen::Constant(1),
                                 asmCmpOp);
        asm_code += codeGen::getFromSpecificReg(inst, op_reg_0);
    }
...
std::string codeGen::assignToSpecificReg(Value *val, int target)
{
    ...
    else if (val_global)
    {
        asm_code += InstGen::getAddress(
            ...
            InstGen::Label(global_vars_label,
    ...
    else if (register_mapping.count(val) && register_mapping.at(val) > 0)
    {
        ...
        {
            asm_code +=
                InstGen::mov(InstGen::Reg(target), InstGen::Reg(source));
        }
    }
    else if (stack_mapping.count(val))
    {
        ...
        if (offset > 4095)
        {
            asm_code +=
                InstGen::mov(InstGen::Reg(op_reg_2), InstGen::Constant(offset));
            asm_code += InstGen::add(InstGen::Reg(op_reg_2),
                                     InstGen::Reg(op_reg_2), InstGen::sp);
            asm_code += InstGen::load(InstGen::Reg(target),
                                      InstGen::Addr(InstGen::Reg(op_reg_2), 0));
        }
        else
    ...
    else if (stack_mapping.count(val))
    {
        ...
        if (offset > 4095)
        {
            ...
            asm_code += InstGen::store(
                InstGen::Reg(source), InstGen::Addr(InstGen::Reg(op_reg_2), 0));
        }
        else
        {
...
int codeGen::queryGOT(Value *val)
{
    return this->GOT.at(val);
}
